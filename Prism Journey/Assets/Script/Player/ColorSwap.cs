...
public class ColorSwap : MonoBehaviour
{
    ...
    private void Update()
    {
        if (playerInputSystem.interactionButton && canColorSwap)
        {
            TrySwap();
            canColorSwap = false;   
        ...
        if (Input.GetMouseButtonDown(0) && canColorSwap)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hit, 150f, colorInteractableLayer))
            {
    ...
    public void TrySwapWithCursor(Transform targeting)
    {
        ...
        if (target == null) 
        {
            PlayerSoundManager.PlaySound(PlayerSoundType.InteractFail);
            return; 
        }
        StartCoroutine(PlayEffect(1));
        ColorIdentity oldColor = target.GetColorIdentity();
